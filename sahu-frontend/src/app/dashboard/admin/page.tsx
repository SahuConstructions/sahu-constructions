...
export default function AdminOverview() {
    ...
    try {
      ...
      setStats({
        ...
        pendingReimbursements: reimbursements.filter(
          (r: any) => r.status === "PENDING_FINANCE" || r.status === "PENDING_HR" || r.status === "PENDING_MANAGER"
        ).length,
      });
      ...
      // Calculate system health (% of employees with complete records)
      const health = emp.data.length > 0
        ? Math.round((filteredTimesheets.length / (emp.data.length * 20)) * 100)
        : 0;
      setSystemHealth(Math.min(health, 100));

      // Weekly trend
      const weekData = generateWeeklyTrend(filteredTimesheets);
  ...
  const cards = [
    ...
    },
    {
      ...
      link: "/dashboard/admin/timesheets",
    },
    {
      title: "Add Employee",
      value: "+",
      ...
      link: "/dashboard/admin/add-employee",
    },
    {
      title: "Payroll",
      ...
      <div className="bg-white rounded-2xl shadow-md p-6 border border-gray-100 flex flex-col md:flex-row justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">
            Admin Overview — {dayjs(`${year}-${month}-01`).format("MMMM YYYY")}
          </h1>
