...
export default function AdminTimesheetsPage() {
    ...
    const updateTimesheetStatus = async (
        ...
        try {
            // Admin override or standard approval
        ...
        <div className="space-y-8 p-6">
            ...
            <div className="rounded-2xl border bg-white shadow-sm p-6 flex justify-between items-center">
                <div>
                    ...
                    <p className="text-sm text-gray-500">
                        Review and finalize all timesheets.
                    </p>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <SummaryCard
                    ...
                    value={timesheets.filter(t => t.status === "PendingHR" || t.status === "PendingManager").length}
            ...
            <div className="space-y-4">
                <h2 className="text-lg font-semibold text-gray-800 flex items-center gap-2">
                    ...
                    <span className="bg-yellow-100 text-yellow-800 text-xs px-2 py-0.5 rounded-full">
                        {timesheets.filter(t => t.status === "PendingHR" || t.status === "PendingManager").length}
                    </span>
                </h2>
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    {timesheets.filter(t => t.status === "PendingHR" || t.status === "PendingManager").length === 0 ? (
                        ...
                        <Table
                            ...
                            rows={timesheets
                                .filter(t => t.status === "PendingHR" || t.status === "PendingManager")
                                .map((t) => [
                            ...
                            rows={timesheets
                                ...
                                .map((t) => [
                                    ...
                                    <div className="flex gap-2">
                                        <button
                                            onClick={() => updateTimesheetStatus(t.id, "approve")}
                                            className="flex items-center gap-1 bg-green-700 hover:bg-green-600 text-white px-3 py-1 rounded-md text-xs font-semibold transition opacity-70 hover:opacity-100"
                                        ...
                                        <button
                                            onClick={() => updateTimesheetStatus(t.id, "reject")}
                                            className="flex items-center gap-1 bg-red-700 hover:bg-red-600 text-white px-3 py-1 rounded-md text-xs font-semibold transition opacity-70 hover:opacity-100"
...
function StatusBadge({ status }: { status: string }) {
    const colors: Record<string, string> = {
        ...
        RejectedByAdmin: "bg-red-800 text-white", // Distinct color for Admin rejection
