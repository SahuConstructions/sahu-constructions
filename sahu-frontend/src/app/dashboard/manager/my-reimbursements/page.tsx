...
export default function ReimbursementsPage() {
  ...
  const submitReimbursement = async (e: React.FormEvent) => {
    ...
    if (!amount || Number(amount) <= 0 || !description.trim()) {
      toast.warning(" Please enter valid amount and description");
      setSubmitting(false);
      return;
    }
    ...
    try {
      ...
      toast.success(" Reimbursement submitted successfully (Pending Manager approval)");
      setAmount("");
      setDescription("");
      setReceipt(null);
      fetchReimbursements();
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <div className="sm:col-span-2 lg:col-span-3 text-center sm:text-right">
          <button
            type="submit"
            disabled={submitting}
            className={`px-6 py-2 rounded-lg text-sm font-medium transition ${submitting
                ...
                reimbursements.map((r, i) => (
                  <tr key={i} className={`${i % 2 === 0 ? "bg-white" : "bg-gray-50"} hover:bg-gray-100 transition`}>
                    ...
                    <td className="py-2 px-3">
                      <span
                        className={`px-2 py-1 rounded-md text-xs font-medium ${r.status === "APPROVED"
      ...
      {showHistory && (
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50 px-3">
          <div className="bg-white p-6 rounded-lg shadow-2xl w-full max-w-6xl max-h-[80vh] overflow-y-auto relative">
