  ...
  const fetchDailyHours = async (selectedDate: string) => {
    try {
      ...
      if (res.data?.hours > 0) {
        ...
        setAutoFetched(true);
        toast.success(` Auto-filled ${res.data.hours} hours from attendance`);
      } else {
        setAutoFetched(false);
        setHours("");
        toast.warning(" No attendance data found for this date");
      }
    } catch {
      setAutoFetched(false);
      setHours("");
      ...
      >
        ...
        <div className="sm:col-span-2 lg:col-span-4 text-center sm:text-right">
          <button
            type="submit"
            disabled={loading}
            className={`bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded-lg text-sm font-medium transition ${loading ? "opacity-70 cursor-not-allowed" : ""
              }`}
          >
